 ...
 *
 * $Log: ipcTest1.java,v $
 * Revision 2.6  2013/07/24 20:01:01  reids
 * Updating lisp, java, python test programs to adhere to updated API
 *
 * Revision 2.5  2013/07/23 21:14:25  reids
 * Made API (relatively) consistent between all languages; added multi-language test procedures
 *
...
public class ipcTest1 {
  ...
  */
  ...
  public static class msg1Handler implements IPC.HANDLER_TYPE {
    public void handle (MSG_INSTANCE msgInstance, Object callData,
			Object clientData) {
      System.out.println("msg1Handler: Receiving message "+ 
			 IPC.IPC_msgInstanceName(msgInstance)+" of "+
			 IPC.IPC_dataLength(msgInstance)+" bytes");
  ...
  public static class queryHandler implements IPC.HANDLER_TYPE {
    public void handle (MSG_INSTANCE msgInstance, Object callData,
      ...
      IPC.IPC_respondData(msgInstance, MSG2, responseStr);
    }
  }
  ...
  public static class replyHandler implements IPC.HANDLER_TYPE {
    public void handle (MSG_INSTANCE msgInstance, Object callData,
      ...
      System.out.println("replyHandler: Receiving message "+
			 IPC.IPC_msgInstanceName(msgInstance)+" ("+callData+") ["+
			 clientData+"]");
      }
  }
  ...
  public static class query2Handler implements IPC.HANDLER_TYPE {
    public void handle (MSG_INSTANCE msgInstance, Object callData,
      ...
      System.out.println("IPC.IPC_respondData("+ RESPONSE2_MSG +", "+
			 matrixString(matrix)+")");
      IPC.IPC_respondData(msgInstance, RESPONSE2_MSG, matrix);
    }
  ...
  public static class stdinHnd implements IPC.FD_HANDLER_TYPE {
	...
	if (in == 'q') {
	  ...
	  IPC.IPC_publishData(MSG2, "Forwarding");
	} else if (in == 'u') {
  ...
  public static class handlerChangeHandler implements IPC.CHANGE_HANDLER_TYPE {
      public void handle (String msgName, int numHandlers, Object clientData) {
	System.out.println("There are now "+ numHandlers 
			   +" handlers for message "+ msgName);
      }
  }

  public static class connectionHandler implements IPC.CONNECT_HANDLER_TYPE {
      public void handle (String moduleName, Object isConnect) {
	System.out.println(moduleName +" just "+
			   ((boolean)isConnect ? "connected" : "disconnected")
  ...
  public static void main (String args[]) throws Exception {
    ...
    IPC.IPC_setContext(IPC.IPC_getContext());
    ...
    System.out.println("\nIPC.IPC_addTimer(1000, TRIGGER_FOREVER, "+
		       " new timerHandler(\"timer1\", "+now+"))");
    IPC.IPC_addTimer(1000, IPC.TRIGGER_FOREVER, new timerHandler("timer1", now));

    // Does not make sense to define a fixed-length Java message 
    // Define a variable length message (simple format string)
    System.out.println("\nIPC.IPC_defineMsg("+ MSG1 +", "+ STRING_FORMAT +")");
    IPC.IPC_defineMsg(MSG1, IPC.IPC_VARIABLE_LENGTH, STRING_FORMAT);
    IPC.IPC_msgClass(MSG1, String.class);
    // Define a variable length message (simple format string)
    System.out.println("\nIPC.IPC_defineMsg("+ MSG2 +", "+ STRING_FORMAT +")");
    IPC.IPC_defineMsg(MSG2, IPC.IPC_VARIABLE_LENGTH, STRING_FORMAT);
    IPC.IPC_msgClass(MSG2, String.class);

    System.out.println("\nIPC.IPC_isMsgDefined("+ MSG1 +") => "+
		       IPC.IPC_isMsgDefined(MSG1));
    System.out.println("\nIPC.IPC_isMsgDefined("+ MSG3 +") => "+
		       IPC.IPC_isMsgDefined(MSG3));

    // Subscribe to the first message, with client data
    System.out.println("\nIPC.IPC_subscribeData("+ MSG1 
		       +", new msg1Handler(client1a))");
    msg1Handler msgHandler1 = new msg1Handler();
    IPC.IPC_subscribeData(MSG1, msgHandler1, "client1a");

    // Subscribe to the second message, with client data
    System.out.println("\nIPC.IPC_subscribeData("+ MSG2
		       +", new msg1Handler(client2a))");
    IPC.IPC_subscribeData(MSG2, new msg1Handler(), "client2a");
    ...
    System.out.println("\nIPC.IPC_publishData("+ MSG1 +", "+ msgData +")");
    if (IPC.IPC_publishData(MSG1, msgData) == IPC_RETURN_TYPE.IPC_OK)
	IPC.IPC_listenClear(1000);

    msgData = "abcde";
    System.out.println("\nIPC.IPC_publishData("+ MSG2 +", "+ msgData +")");
    if (IPC.IPC_publishData(MSG2, msgData) == IPC_RETURN_TYPE.IPC_OK)
	IPC.IPC_listenClear(1000);

    // Get notified when handlers subscribe/unsubscribe to "msg2"
    System.out.println("\n"+ IPC.IPC_numHandlers(MSG2) 
		       +" handlers currently subscribed to "+ MSG2);
    System.out.println("\nIPC.IPC_subscribeHandlerChange("+MSG2
		       +", new handlerChangeHandler())");
    IPC.IPC_subscribeHandlerChange(MSG2, new handlerChangeHandler(), null);

    // Subscribe a second message handler for "msg2"
    System.out.println("\nIPC.IPC_subscribeData("+ MSG2
		       +", new msg2Handler(client2b))");
    IPC.IPC_subscribeData(MSG2, new msg2Handler(), "client2b");
    ...
    if (IPC.IPC_publishData(MSG2, msgData) == IPC_RETURN_TYPE.IPC_OK) {
      ...
      while (IPC.IPC_listen(1000) != IPC_RETURN_TYPE.IPC_Timeout) {}
    }
    ...
    IPC.IPC_unsubscribe(MSG2, msg2Handler.class);
    // If doing direct broadcasts, need to listen to get the direct info update
    IPC.IPC_listen(250);

    // No longer get notified when handlers are added/removed
    System.out.println("\nIPC.IPC_unsubscribeHandlerChange("+ MSG2
		       +", handlerChangeHandler.class)");
    IPC.IPC_unsubscribeHandlerChange(MSG2, handlerChangeHandler.class);
    System.out.println("  IPC.IPC_subscribeData("+ MSG2
		       +", new msg2Handler(client2c))");
    IPC.IPC_subscribeData(MSG2, new msg2Handler(), "client2b");
    System.out.println("  IPC.IPC_unsubscribe("+ MSG2 +", msg2Handler)");
    IPC.IPC_unsubscribe(MSG2, msg2Handler.class);
    ...
    IPC.IPC_subscribeData(MSG1, msgHandler1, "client1b");
    ...
    IPC.IPC_unsubscribe(MSG1, msgHandler1);
    // If doing direct broadcasts, need to listen to get the direct info update
    IPC.IPC_listen(250);

    // Receive no messages -- IPC.IPC_listenClear times out
    System.out.println("\nIPC.IPC_publishData("+ MSG1 +", "+ msgData +")");
    if (IPC.IPC_publishData(MSG1, msgData) == IPC_RETURN_TYPE.IPC_OK) { 
	if (IPC.IPC_listen(1000) == IPC_RETURN_TYPE.IPC_Timeout) {
    ...
    System.out.println("\nIPC.IPC_subscribeData("+ QUERY_MSG
		       +", new queryHandler(), 'qtest')");
    IPC.IPC_subscribeData(QUERY_MSG, new queryHandler(), "qtest");
    System.out.println("\nIPC.IPC_defineMsg("+ QUERY_MSG +", "+ STRING_FORMAT +")");
    IPC.IPC_defineMsg(QUERY_MSG, IPC.IPC_VARIABLE_LENGTH, STRING_FORMAT);
    IPC.IPC_msgClass(QUERY_MSG, String.class);
    ...
    System.out.println("\nIPC.IPC_queryNotifyData("+ QUERY_MSG +", 'abcdef', "+
		       "new replyHandler('Notification')");
    IPC.IPC_queryNotifyData(QUERY_MSG, "abcdef", new replyHandler(),
			    "Notification");
    ...
    msgData = "abcdef";
    System.out.println("\nIPC.IPC_queryResponseData("+ QUERY_MSG +", "+ msgData
		       +", String.class, IPC.IPC_WAIT_FOREVER)");
    Object response = IPC.IPC_queryResponseData(QUERY_MSG, msgData,
    ...
    System.out.println("\nIPC.IPC_queryResponseData("+ QUERY_MSG +", "+ msgData
		       +", String.class, 0)");
    response = IPC.IPC_queryResponseData(QUERY_MSG, msgData, 0);
    if (response != null) {
	System.out.println("Blocking Response: "+ response);
    } else {
	...
	//     occurs in IPC.IPC_msgFormatter, below).
    ...
    System.out.println("\nIPC.IPC_defineMsg("+ MSG3 +", "+ INT_FORMAT+")");
    IPC.IPC_defineMsg(MSG3, IPC.IPC_VARIABLE_LENGTH, INT_FORMAT);
    IPC.IPC_msgClass(MSG3, int.class);
    // Define a variable-length message whose format is a string
    System.out.println("\nIPC.IPC_defineMsg("+ MSG4  +", "+ STRING_FORMAT+")");
    IPC.IPC_defineMsg(MSG4, IPC.IPC_VARIABLE_LENGTH, STRING_FORMAT);
    IPC.IPC_msgClass(MSG4, String.class);
    // Define a variable-length message whose format is a complex structure
    System.out.println("\nIPC.IPC_defineMsg("+ MSG5 +", "+  MATRIX_LIST_FORMAT+")");
    IPC.IPC_defineMsg(MSG5, IPC.IPC_VARIABLE_LENGTH, MATRIX_LIST_FORMAT);
    IPC.IPC_msgClass(MSG5, matrix_list.class);

    // Subscribe to each of the above messages, all using the same handler
    //   Differentiated by the dataClass argument...
    System.out.println("\nIPC.IPC_subscribeData("+ MSG3 +", new msg3Handler())");
    IPC.IPC_subscribeData(MSG3, new msg3Handler(), int.class);
    System.out.println("\nIPC.IPC_subscribeData("+ MSG4 +", new msg3Handler())");
    IPC.IPC_subscribeData(MSG4, new msg3Handler(), String.class);
    System.out.println("\nIPC.IPC_subscribeData("+ MSG5 +", new msg3Handler())");
    IPC.IPC_subscribeData(MSG5, new msg3Handler(), matrix_list.class);

    // Publish integer data
    int ivalue = 42;
    System.out.println("\nIPC.IPC_publishData("+ MSG3  +", "+ ivalue+")");
    IPC.IPC_publishData(MSG3, ivalue);
    IPC.IPC_listenClear(1000);

    // Publish string data
    msgData = "Hello, world";
    System.out.println("\nIPC.IPC_publishData("+ MSG4  +", "+ msgData+")");
    IPC.IPC_publishData(MSG4, msgData);
    IPC.IPC_listenClear(1000);
    ...
    // Use of IPC.IPC_queryResponseData and IPC.IPC_respondData -- 
    ...
    // IPC.IPC_queryResponseData both marsalls and sends the data structure
    System.out.println("\nIPC.IPC_queryResponseData("+ 
		       QUERY2_MSG  +", "+ m1  +", IPC.IPC_IPC.IPC_WAIT_FOREVER)"); 
    Object matrixReply = IPC.IPC_queryResponseData(QUERY2_MSG, m1,
						   IPC.IPC_WAIT_FOREVER);
    if (matrixReply != null)
      System.out.println("response data: "+ 
			 query2Handler.matrixString((float[][])matrixReply));
    else
      System.out.println("IPC.IPC_queryResponseData failed");
    ...
    System.out.println("\nIPC.IPC_subscribeFD(0, new stdinHnd(FD1))");
    IPC.IPC_subscribeFD(0, new stdinHnd(), "FD1");
